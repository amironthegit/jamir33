...
def extract_new_configs(html, last_post_id):
    ...
    for post_id, text_block in reversed(blocks):
        if last_post_id and post_id == last_post_id:
            print(f"Reached last processed post: {post_id}")
            break
...
def write_index_html(count, updated_at):
    html = (
        '<!DOCTYPE html><html><head><meta charset="utf-8">'
        '<title>V2Ray Sub</title></head><body>'
        '<h2>V2Ray Subscription Active</h2>'
        '<p>Configs: ' + str(count) + '</p>'
